  ...
  constructor() {
    ...
    this.methods = {
        ...
        if (items.length > 0) {
          items.forEach(item => {
            !item.data.checkAll &&
              item.setData({
                checked:
                  value === null || value === void 0 ? void 0 : value.includes(item.data.value),
              })
          })
      ...
      handleInnerChildChange(e) {
        ...
        if (item.checkAll) {
          rect.indeterminate =
            (_a = this.$checkAll) === null || _a === void 0 ? void 0 : _a.data.indeterminate
        }
      ...
      setCheckall() {
        ...
        const isCheckall = items.every(item =>
          item.data.checkAll ? true : valueSet.has(item.data.value)
        )
